...
function ckeditor_commitPage($restOfPath){
    ...
    if((array_key_exists("noPage", $row))&& $current_user->permissionContains("aurora_pages", "true")){  
    	...
    	$result = mysql_query("UPDATE `pages` SET `content`='$pageContent' WHERE `title`='$pageName' LIMIT 1;", getPrimarySQLConnection());
        echo "UPDATE `pages` SET `content`='$pageContent' WHERE `title`='$pageName' LIMIT 1;<br />".mysql_affected_rows($result)."<br />";    
    }
