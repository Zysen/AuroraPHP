...
class PhpClosure {
  ...
  function setExternsURL($url) {
    $this->_externsUrl = $url;
    return $this;
  }
  ...
  function write() {
    ...
    if ($this->_cache_dir == "") {
      $outputData =  $this->_compile();
      //AuroraLog("CLOSURE_COMPILE", $outputData);
    } else {
  ...
  function _compile() {
    ...
    if(strlen($errors)>0)
        AuroraLog("CLOSURE_COMPILE_ERRORS", $errors);
    if(strlen($warnings)>0)  
        AuroraLog("CLOSURE_COMPILE_WARNINGS", $warnings);
    if(strlen($result)>0)  
        AuroraLog("CLOSURE_COMPILE_DEBUG", $result);
    return array(compiledCode=>$code, errors=>$errors, warnings=>$warnings, debug=>$result);
  }
  ...
  function _getParamList() {
    ...
    if($this->_externsUrl!=""){
        $params["externs_url"] = $this->_externsUrl;
    }
